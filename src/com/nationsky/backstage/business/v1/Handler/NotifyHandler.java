...
public class NotifyHandler {
	 ...
	 * @param taskId 任务Id
	 * @param type 通知类型: 1收到任务邀请类型, 2任务被拒绝类型, 3任务被同意类型, 4任务被删除类型, 5任务已完成类型, 6任务延期类型(还没写), 7好友添加通知, 8任务修改类型(还没写), 9退出任务类型, 10同意好友添加通知, 11拒绝好友添加通知,12取消完成任务,13成员被移除
	 * @return
	 */
...
//				for (String pushToken : pushTokens.split(" ")) {
					...
					if(type == 1){
						...
						content =  "您已从"+taskInfo.getTitle()+"任务中被移除";
					}
